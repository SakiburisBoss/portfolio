...
export const ProjectDetailPage: React.FC<ProjectDetailState> = ({
  ...
  // Modern iframe state management
  ...
  // Reset iframe function
  const resetIframe = () => {
    console.log("🔄 Resetting iframe...");
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    if (intersectionRef.current) intersectionRef.current.disconnect();
  ...
  // Create iframe after delay to prevent initial browser error display
  useEffect(() => {
    if (!project.liveDemoUrl) return;

    console.log("🔄 Starting iframe creation delay...");
    const timer = setTimeout(() => {
      console.log("✅ Delay complete, creating iframe");
      setShowIframe(true);
  ...
  // Advanced iframe detection using multiple techniques
  useEffect(() => {
    if (!showIframe || !iframeRef.current || !project.liveDemoUrl) return;

    console.log(
      "🔍 Starting advanced iframe detection for:",
      project.liveDemoUrl
    );
    ...
    // Method 1: Intersection Observer for visibility detection
    intersectionRef.current = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasLoaded && !hasTimedOut) {
            console.log("👁️ Iframe is intersecting viewport");
          }
        });
      },
      { threshold: 0.1 }
    ...
    if (iframeRef.current) {
      intersectionRef.current.observe(iframeRef.current);
    }

    // Method 2: Content detection through postMessage attempt
    const checkIframeContent = () => {
      if (!iframeRef.current || hasLoaded || hasTimedOut) return;

      try {
        // Try to access iframe content window (will fail for blocked iframes)
        const iframeWindow = iframeRef.current.contentWindow;
        if (iframeWindow) {
          // Attempt to send a message - blocked iframes won't receive this
          iframeWindow.postMessage("test", "*");
          console.log("📤 PostMessage sent to iframe");
        }
      } catch (error) {
        console.log("🚫 Iframe content access blocked:", error);
        if (!hasLoaded) {
          hasTimedOut = true;
    ...
    // Method 3: Load event detection (fires even for blocked content)
    const handleLoad = () => {
      console.log("📡 Iframe load event fired");

      // Additional verification - check if we can access basic properties
      setTimeout(() => {
        ...
        try {
          ...
          if (iframe && iframe.contentDocument) {
            console.log("✅ Iframe content accessible - working");
            hasLoaded = true;
            setIframeStatus("working");
          } else {
            console.log("🔒 Iframe content blocked - checking further");
            checkIframeContent();
          }
        } catch {
          console.log("❌ Iframe access denied - failed");
          if (!hasLoaded) {
            hasTimedOut = true;
            setIframeStatus("failed");
          }
    ...
    timeoutRef.current = setTimeout(() => {
      if (!hasLoaded) {
        console.log("⏰ Iframe timeout reached - considering failed");
        hasTimedOut = true;
        setIframeStatus("failed");
      }
    ...
    if (iframe) {
      ...
      // Also try to detect when iframe starts loading
      iframe.addEventListener("loadstart", () => {
        console.log("🚀 Iframe loadstart event");
      });
    }

    // Check content periodically for the first few seconds
    const contentCheckInterval = setInterval(checkIframeContent, 1000);
    setTimeout(() => clearInterval(contentCheckInterval), 3000);
                ...
                >
                  {project.liveDemoUrl ? (
                    <div className="relative w-full h-full">
                            ...
                            <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-4 text-left">
                              ...
                              <ul className="text-xs space-y-1 text-gray-600 dark:text-gray-400">
                                <li>
                                  ...
                                  blocking iframe embedding
                                </li>
                                <li>
                                  • Site explicitly prohibits iframe display for
                                  security
                                </li>
                                <li>
                                  • Authentication or login required to view
                                  content
                                </li>
                                <li>
                                  • HTTPS/SSL certificate issues or mixed
                                  content blocking
                                </li>
                                <li>
                                  • Slow network connection or server response
                                  timeout
                                </li>
                                <li>
                                  • Server temporarily unavailable or under
                                  maintenance
                                </li>
                                <li>
                                  • Geolocation restrictions or content not
                                  available in your region
                                </li>
                                <li>
                                  • Browser security policies preventing
                      ...
                      {/* Conditional iframe rendering - only create when needed */}
